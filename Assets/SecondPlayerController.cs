...
public class SecondPlayerController : MonoBehaviour
{
    ...
    private void CanMove()
    {
        ...
        if (movementStyle == MovementStyle.Physical)
        {
            if (horizontalMovement)
                _horizontalInput = Input.GetAxis(Axis.Horizontal2);
        
            if (verticalMovement)
                _verticalInput = Input.GetAxis(Axis.Vertical2);
        }
        else if (movementStyle == MovementStyle.Linear)
        {
            if (horizontalMovement)
                _horizontalInput = Input.GetAxisRaw(Axis.Horizontal2);
        
            if (verticalMovement)
                _verticalInput = Input.GetAxisRaw(Axis.Vertical2);
        }
    ...
    private void CanJump()
    {
        ...
        _jumpDown = Input.GetButtonDown(Button.Jump2);
        _jump = Input.GetButton(Button.Jump2);
        _jumpUp = Input.GetButtonUp(Button.Jump2);
